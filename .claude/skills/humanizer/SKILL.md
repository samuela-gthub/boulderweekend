...
The patterns hold in every language. For Dutch text, treat "niet X, maar Y", "het gaat niet om X, het gaat om Y", "laten we eens kijken", "cruciaal", "essentieel", "naadloos", "in een notendop" and "kortom" the way you would treat their English equivalents. Dutch writing uses fewer dashes and less title case than English, so a dash or a capitalized heading is a stronger tell there.
...
## F. Page structure and markup

These come from the parts of Wikipedia's list that are about the shape of a page instead of its sentences.

### 26. Tables and headings with nothing in them

**Watch for:** a two-column table with three or four rows that would read better as one sentence; a heading followed directly by another heading with no text of its own; skipped heading levels (a page that jumps from `#` to `###`); several top-level headings in one document.
**Problem:** The model formats by habit. A short table or an empty parent heading adds structure without adding content. Turn the small table into prose, give the parent heading a sentence or remove it, and keep heading levels in order. Keep a table when the reader will compare values across rows.
**Before:**
> ## Practical Info
> ### Costs
> | Item | Price |
...
**After:**
> ## Practical info
> Entry is €15, and shoes rent for €4.

### 27. Placeholders and leaked markup

**Watch for:** unfilled template slots such as `[Your Name]`, `[Insert date]`, `[Specific Topic]`; citation artifacts such as `:contentReference[oaicite:0]`, `oai_citation`, `turn0search0`, `[cite: 1]`, `grok_card`, lenticular brackets 【】 around sources; Markdown syntax (`**bold**`, `## heading`) left raw in a format that does not render it, such as HTML, plain-text email, or a wiki.
**Problem:** These are certain tells: nobody types them by hand. Fill a placeholder only when the user or source gives the value; otherwise ask. Delete citation artifacts, keeping a real link if one is recoverable. Convert stray Markdown to the target format.

## G. Story structure (StoryScope)

Source: Russell et al., "StoryScope: Investigating idiosyncrasies in AI fiction" (arXiv:2604.03136, 2026). The paper compared 61,608 stories written by people and five models for the same prompts. Narrative choices alone separated human from AI stories with 93% macro-F1, and removing surface phrasing barely changed the result. So these tells survive a sentence-level cleanup. Fix them by changing the structure.

They matter most in fiction, personal essays, newsletters, trip reports, and any text that tells what happened. In factual text, change order and emphasis only: never invent an event, a loose end, a reference, or a feeling to make the structure look human.

### 28. The narrator states the lesson

**Watch for:** a closing paragraph that names the theme ("In the end, she learned that grief..."); dialogue that turns into a debate about ideas; a moral question placed at the center of an ordinary story. In the paper, AI narrators spelled out the theme in 77% of stories against 52% for people.
**Problem:** The text explains its meaning instead of trusting the reader to get it. Cut the stated lesson and end on the last event, image, or line of dialogue that carries it. In a report or essay, end on the last concrete fact, as in §13.
**Before:**
> He left the gym with torn skin and no top. Somewhere on the drive home he understood that the climb had never really been about the summit, but about the people who stood beneath him.
**After:**
> He left the gym with torn skin and no top. Sander drove, and nobody mentioned the project until Breda.

### 29. Tidy, linear, single-track

**Watch for:** events told strictly from first to last; every thread resolved; a protagonist whose own choice fixes everything; an ending in quiet acceptance or understanding; no subplot, no time jump, no flashback. AI stories had no subplot 79% of the time (people: 57%) and ended in internal acceptance 47% of the time (people: 27%).
**Problem:** Real accounts are messier. When the material allows it, open closer to the most interesting moment and fill in the lead-up afterward; leave a question open when the source leaves it open; let outside events decide things when that is what happened. Do not make up a subplot or an open end that did not exist.

### 30. Emotion only through the body and the weather

**Watch for:** fear as a tight chest and cold sweat, grief as a heavy stomach, rain or dim light that mirrors a mood, smell in every scene. AI used bodily metaphors for emotion in 81% of stories (people: 38%) and named a feeling outright only 8% of the time (people: 29%).
**Problem:** The body-and-setting routine is the model's default, so it reads as a template. Name the feeling plainly some of the time ("he was scared"), cut sensory detail that does no work, and let the setting be only the setting.
**Before:**
> Her throat tightened as the grey sky pressed down on the wall, the smell of chalk and rubber thick in the air.
**After:**
> She was nervous. The wall was grey, and the place smelled of chalk.

### 31. Vague allusions and no reader

**Watch for:** "like a hero from an old myth", "as the poets say", references that name no book, band, place, or brand; a narrator who never admits a reader exists. People named specific works nearly twice as often (47% against 24%) and addressed the reader four times as often (28% against 7%).
**Problem:** A vague allusion signals culture without committing to any. Name the thing when the writer or source names it, or cut the allusion. Where the voice allows, speak to the reader directly. Never invent a reference.

### 32. Flat escalation and one voice

**Watch for:** stakes that stay level from start to finish; every character talking in the same measured register; an epilogue that settles everything gently. The paper found Claude's stories had the flattest escalation and the most uniform voice of all sources, so check for this whenever Claude wrote the draft.
**Problem:** Uniform intensity reads as careful and lifeless. Let the stakes rise where the material rises, give characters different ways of talking, and drop an epilogue that only reassures.
...
The patterns come from Wikipedia's ["Signs of AI writing"](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing), maintained by WikiProject AI Cleanup, and from reviews of AI-generated text on Wikipedia and elsewhere. Sections A to E are the humanizer skill by Siqi Chen (https://github.com/blader/humanizer, MIT License, see LICENSE). Section F adds the remaining page-structure and markup tells from the same Wikipedia page. Section G comes from Russell et al., ["StoryScope: Investigating idiosyncrasies in AI fiction"](https://arxiv.org/abs/2604.03136) (2026).
